classdef EyeMovementGratingSurround < edu.washington.riekelab.protocols.RiekeLabStageProtocol
    ...
    properties
        ...
        gratingBarWidth = 80 % um
        gratingContrast = 0.5 % fraction of background
    ...
    methods
        ...
        function p = createPresentation(obj)
            ...
            gratingBarWidthPix = obj.rig.getDevice('Stage').um2pix(obj.gratingBarWidth);
            ...
            if (obj.centerDiameter > 0) %% Create center aperture
                aperture = stage.builtin.stimuli.Rectangle();
                aperture.position = canvasSize/2;
                aperture.color = obj.backgroundIntensity;
                aperture.size = 2.*[max(canvasSize) max(canvasSize)];
                mask = stage.core.Mask.createCircularAperture(centerDiameterPix/(2*max(canvasSize)), 1024); %circular aperture
                aperture.setMask(mask);
                p.addStimulus(aperture);
            end

            %grating background
            grate = edu.washington.riekelab.turner.stimuli.GratingWithShift('square'); %square wave grating
            grate.size = 2.*[max(canvasSize) max(canvasSize)];
            grate.position = canvasSize/2;
            grate.spatialFreq = 1/(2*gratingBarWidthPix); %convert from bar width to spatial freq
            grate.color = 2*obj.backgroundIntensity;
            grate.contrast = obj.gratingContrast;
            grate.shiftX = 0.5;

            grateMask = stage.core.Mask.createCircularAperture(centerDiameterPix/(2*max(canvasSize)), 1024); %circular aperture
            grate.setMask(grateMask);

            maxIntensity = obj.backgroundIntensity + (obj.gratingContrast * obj.backgroundIntensity);
            if maxIntensity > 1.0
               error('Combination of grating contrast & background too high!') 
            end
            
            %control grating shift
            %apply eye trajectories to move image around
            grateShiftX = stage.builtin.controllers.PropertyController(grate,...
                'shiftX', @(state)getShiftX(obj, state.time - obj.preTime/1e3, 2*gratingBarWidthPix));

            function p = getShiftX(obj, time, periodLength)
                if time < 0
                    p = 0;
                elseif time > obj.timeTraj(end) %out of eye trajectory, hang on last frame
                    p = -(obj.xTraj(end)) / periodLength;
                else %within eye trajectory and stim time
                    p = -interp1(obj.timeTraj,obj.xTraj,time) / periodLength;
                end
            end
            p.addController(grateShiftX);
            ...
            if  (obj.apertureDiameter > 0) % Create aperture
                ...
                mask = stage.core.Mask.createCircularAperture(apertureDiameterPix/aperture.size(1), 1024); %circular aperture
                aperture.setMask(mask);
